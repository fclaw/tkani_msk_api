-- Deploy tkani-api:00073.daily_stats_bonuses to pg
...
SELECT
    rs.action_date AS sale_date,
    
    -- Общий доход за день (Заказы + Полка)
    (SUM(line_price_kopecks) / 100.0) - COALESCE(b.total_expended_bonuses, 0) AS total_revenue,
...
FROM all_revenue_streams AS rs
LEFT JOIN (
  SELECT
   created_at :: DATE AS  action_date,
   SUM(bonuses) AS total_expended_bonuses
   FROM payments
   WHERE status = 'confirmed'
   GROUP BY created_at :: DATE
) AS b
ON rs.action_date = b.action_date
GROUP BY rs.action_date, b.total_expended_bonuses
