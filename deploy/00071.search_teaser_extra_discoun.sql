-- Deploy tkani-api:00071.search_teaser_extra_discoun to pg
    ...
    (
        SELECT
            ...
            jsonb_build_object(
                ...
                'article', f.article,
                'type', 'roll',
                'price', ROUND(f.price_per_meter * (1 - calculate_total_discount(f.discount, f.lifecycle :: text, f.is_extra_discount_eligible))),
                'discount', f.discount,
    ...
    (
        SELECT
            ...
            jsonb_build_object(
                ...
                'article', f.article,
                'type', 'pre_cut',
                'price', ROUND(pc.price_rub * (1 - calculate_total_discount(f.discount, f.lifecycle :: text, f.is_extra_discount_eligible))),
                'discount', f.discount,
                'thumbnail_url', f.thumbnail_url
            ) :: jsonb AS teaser_json
        FROM 
