...
data LocationNode = LocationNode
  { lnType            :: NodeType               -- ^ "custom_location" (door) OR "platform_station" (PVZ)
  , lnContact         :: ContactInfo
  , lnCustomLocation  :: Maybe CustomLocation   -- ^ Fill this if delivering to a door / sending from your warehouse
  , lnPlatformStation :: Maybe PlatformStation  -- ^ Fill this if sending to a PVZ
  } deriving (Show, Eq, Generic)


data ContactInfo = ContactInfo
  { ciPhone :: Text -- ^ Format: "+79991234567"
  , ciName  :: Text
  , ciEmail :: Maybe Text
  } deriving (Show, Eq, Generic)
...
data BillingInfo = BillingInfo
  ...
  , biDeliveryCost  :: Int  -- ^ Who pays for delivery? 0 if you provide free delivery to client. (usually in kopecks/minor units)
  } deriving (Show, Eq, Generic)
...
data ItemBillingDetails = ItemBillingDetails
  { ibdUnitPrice         :: Int -- ^ Price to be paid by customer (in kopecks/minor units, 100 = 1 ruble)
  , ibdAssessedUnitPrice :: Int -- ^ Declared value for insurance (in kopecks/minor units)
  } deriving (Show, Eq, Generic)

data Place = Place
  { pPhysicalDimensions :: PhysicalDimensions
  } deriving (Show, Eq, Generic)

data PhysicalDimensions = PhysicalDimensions
  { pdDx          :: Int -- ^ Length in cm
  , pdDy          :: Int -- ^ Width in cm
  , pdDz          :: Int -- ^ Height in cm
  , pdWeightGross :: Int -- ^ Weight in Grams
  } deriving (Show, Eq, Generic)


$(deriveJSON defaultOptions { fieldLabelModifier = recordLabelModifier "ri" } ''RequestInfo)
$(deriveJSON defaultOptions { fieldLabelModifier = recordLabelModifier "ci" } ''ContactInfo)
...
$(deriveJSON defaultOptions { fieldLabelModifier = recordLabelModifier "ln" } ''LocationNode)
