...
import Infrastructure.Database (pickupOrdersForShipment, recordCourierPickupFailure, createCourierPickup)
...
import Infrastructure.Services.Sdek.Types.Config (SdekConfig (..), from, to, parseHour)
      ...
      Right orders ->
        if null orders then
          ...
          fmap (const False) $ $(logTM) InfoS $ ls $
            ...
            tshow minCourierPickup <> " for courier pickup. Skipping scheduling."
        else do
